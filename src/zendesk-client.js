...
class ZendeskClient {
  ...
  getBaseUrl() {
    return `https://${this.getCredentials().subdomain}.zendesk.com/api/v2`;
  }

  getAuthHeader() {
    const auth = Buffer.from(`${this.getCredentials().email}/token:${this.getCredentials().apiToken}`).toString('base64');
    return `Basic ${auth}`;
  }

  async request(method, endpoint, data = null, params = null) {
    try {
      if (!this.getCredentials().subdomain || !this.getCredentials().email || !this.getCredentials().apiToken) {
